...
void Conversions::AddItemToQueue(const AdEventInfo& ad_event) {
  ...
  conversion_queue_item.timestamp = now + rand_delay;

  database::table::ConversionQueue database_table;
  database_table.InsertOrUpdate(
      conversion_queue_item, [=](const Result result) {
        if (result != SUCCESS) {
...
bool Conversions::RemoveItemFromQueue(
    const ConversionQueueItemInfo& cconversion_queue_item) {
  database::table::ConversionQueue database_table;
...
void Conversions::ProcessQueueItem(
  ...
  const std::string friendly_date_and_time =
      FriendlyDateAndTime(conversion_queue_item.timestamp);
...
void Conversions::StartTimer(
  ...
  const base::Time timestamp =
      base::Time::FromDoubleT(conversion_queue_item.timestamp);

...
std::string Decrypt(const std::vector<uint8_t>& ciphertext,
  ...
  std::vector<uint8_t> plaintext_slice(plaintext.begin() + crypto_box_ZEROBYTES,
                                       plaintext.end());
...
ConversionIdInfo EncryptAndEncodeConversionId(
  ...
  std::vector<uint8_t> ciphertext =
      Encrypt(padded_message, nonce, advertiser_pk, ephemeral_sk);

  std::cout << "*** ciphertext.size: " << ciphertext.size();

  // The receiving TweetNaCl.js client does not require padding
  std::vector<uint8_t> ciphertext_slice(
      ciphertext.begin() + crypto_box_BOXZEROBYTES, ciphertext.end());

...
// Converts a StorageResult to its corresponding representation in the 2D
// matrix on the QA test page.
std::string AsString(StorageResult r) {
  switch (r) {
...
// Converts a StorageType to its corresponding representation in the 2D
// matrix on the QA test page.
...
// available on Brave's QA test pages, hosted at
// https://github.com/brave-experiments/qa-test-pages
//
// The tests validate expected results by checking against a 4x4 matrix
// populated by the page's Javascript code.
//
...
class EphemeralStorageTest : public InProcessBrowserTest {
 public:
  ...
  void ClickStartTest(content::WebContents* contents) {
    ...
    ExecuteScriptAsync(contents,
                       "document.getElementById('button-start-test').click()");
    std::string message;
  ...
  void ClickReadValues(content::WebContents* contents) {
    content::DOMMessageQueue queue(contents);
    ExecuteScriptAsync(contents,
                       "document.getElementById('button-read-values').click()");
    std::string message;
    while (message != "\"button operation completed\"") {
      ASSERT_TRUE(queue.WaitForMessage(&message));
    }
  ...
  // Checks that the 2D storage results matrix on the test page matches the
  // expected values
  ...
  // Checks a particular row of the 2D storage results matrix
  void CheckStorageResults(content::WebContents* contents,
                           StorageType storage_type,
                           const StorageResult expected[4]) {
    ASSERT_EQ(
        true,
        EvalJs(contents, "document.querySelector('#storage-rs .row-" +
                             AsString(storage_type) + " .cell-this-frame.bg-" +
                             AsString(expected[0]) + "') !== null"));
    ASSERT_EQ(
        true,
        EvalJs(contents, "document.querySelector('#storage-rs .row-" +
                             AsString(storage_type) + " .cell-local-frame.bg-" +
                             AsString(expected[1]) + "') !== null"));
    ASSERT_EQ(true,
              EvalJs(contents, "document.querySelector('#storage-rs .row-" +
                                   AsString(storage_type) +
                                   " .cell-remote-frame.bg-" +
                                   AsString(expected[2]) + "') !== null"));
    ASSERT_EQ(true,
              EvalJs(contents, "document.querySelector('#storage-rs .row-" +
                                   AsString(storage_type) +
                                   " .cell-nested-frame.bg-" +
                                   AsString(expected[3]) + "') !== null"));
  }

  // Corresponds to step 1 of QA test page
  ...
  // Corresponds to step 2 of QA test page
  ...
  // Corresponds to step 3 of QA test page
  ...
  // Corresponds to step 4 of QA test page
  ...
  // Corresponds to step 5 of QA test page
  ...
  // Corresponds to step 6 of QA test page
